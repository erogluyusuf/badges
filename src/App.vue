...
const getRepos = async () => {
  ...
  try {
    ...
    alert("API Error! Could not fetch user data.");
  } finally {
    loading.value = false;
  }
};
...
const fetchRepoImages = async (repo: any) => {
...
const finalImageUrl = computed(() => {
  ...
  return customImageUrl.value || 'https://via.placeholder.com/400x200?text=Enter+Image+URL';
});
...
const addCustomBadge = () => {
  ...
  if (customBadgeLogo.value) {
    url += `&logo=${encodeURIComponent(customBadgeLogo.value)}&logoColor=white`;
  }
  ...
  customBadgeLabel.value = '';
  customBadgeMessage.value = '';
  customBadgeLogo.value = '';
};
...
const downloadImage = async () => {
  ...
  try {
    ...
    alert("Failed to download image. CORS constraints might be active on custom image URLs.");
  } finally {
    ...
    <div class="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6">
      <div class="lg:col-span-5 bg-neutral-900 border border-neutral-800 rounded-2xl p-6 flex flex-col gap-6 max-h-[90vh] overflow-y-auto custom-scrollbar shadow-2xl">
        ...
        <div>
          ...
          <select v-if="repos.length > 0" @change="selectRepo(repos[$event.target.value])" class="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-4 py-3 outline-none focus:border-blue-500 text-sm cursor-pointer appearance-none">
        ...
        <div v-if="selectedRepo" class="space-y-4">
          ...
          <div class="bg-neutral-950/50 p-3 rounded-xl border border-neutral-800">
            ...
            <div v-if="imageSource === 'repo'" class="animate-fade-in bg-neutral-900 border border-neutral-800 rounded-lg p-3 mb-3">
              ...
              <div v-else-if="repoImages.length === 0" class="text-center py-4 text-xs text-neutral-500">No images found.</div>
              <div v-else class="grid grid-cols-3 gap-2 max-h-48 overflow-y-auto custom-scrollbar pr-1">
                <div v-for="img in repoImages" :key="img.path" @click="selectedRepoImage = img.url; resetImageSettings()"
                     :class="['relative rounded-md cursor-pointer border-2 h-16 bg-neutral-800', selectedRepoImage === img.url ? 'border-blue-500 scale-105' : 'border-transparent']">
            ...
            <div class="mt-4 pt-3 border-t border-neutral-800 space-y-3">
              <div class="flex justify-between items-center">
                <span class="text-[10px] font-bold text-neutral-500 uppercase tracking-widest">Image Adjustments</span>
                <button @click="resetImageSettings" class="text-[10px] text-blue-500 font-bold">Reset</button>
              </div>
          ...
          <div class="bg-neutral-950/50 p-4 rounded-xl border border-blue-900 space-y-3 shadow-lg shadow-blue-950/20 relative overflow-hidden">
            <div class="absolute -top-10 -right-10 w-24 h-24 bg-blue-600/10 blur-2xl rounded-full"></div>
            ...
            <div class="flex items-center gap-2 relative z-10">
              ...
              <button @click="addCustomBadge" class="bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold px-4 py-2 h-[34px] rounded-lg transition-all shadow-lg active:scale-95 shrink-0">Add</button>
            </div>
            <p class="text-[9px] text-neutral-500 mt-1 relative z-10">
              💡 Use <a href="https://simpleicons.org/" target="_blank" class="text-blue-400 hover:underline">Simple Icons</a> slugs for icons (e.g., github, react).
            </p>
          ...
          <div class="mt-4 pt-3 border-t border-neutral-800 space-y-3">
            ...
            <div>
              <div class="flex justify-between text-[10px] text-neutral-400 mb-1">
                <span>Card Width</span><span>{{ config.cardWidth }}px</span>
              </div>
              <input type="range" v-model.number="config.cardWidth" min="350" max="800" step="10" class="w-full accent-blue-500">
            </div>
            <div>
              <div class="flex justify-between text-[10px] text-neutral-400 mb-1">
                <span>Inner Padding</span><span>{{ config.cardPadding }}px</span>
              </div>
          ...
          <div class="mt-4 pt-3 border-t border-neutral-800 space-y-3">
            <div class="flex justify-between items-center mb-2">
              <span class="text-[10px] font-bold text-neutral-500 uppercase tracking-widest">Color Palette</span>
              <button @click="resetColors" class="text-[10px] text-blue-500 font-bold">Reset</button>
            </div>
            <div class="grid grid-cols-3 gap-2">
              <div class="flex flex-col gap-1">
                <label class="text-[9px] text-neutral-400 uppercase text-center">Background</label>
                <input type="color" v-model="config.cardBg" class="w-full h-8 rounded cursor-pointer border-0 bg-transparent p-0">
              </div>
              <div class="flex flex-col gap-1">
                <label class="text-[9px] text-neutral-400 uppercase text-center">Text</label>
                <input type="color" v-model="config.cardText" class="w-full h-8 rounded cursor-pointer border-0 bg-transparent p-0">
              </div>
              <div class="flex flex-col gap-1">
                <label class="text-[9px] text-neutral-400 uppercase text-center">Image Canvas</label>
          ...
          <div>
            <label class="block text-xs text-neutral-400 mb-2">Display Statistics</label>
            <div class="grid grid-cols-2 md:grid-cols-3 gap-2 text-[10px]">
              ...
              <label class="flex items-center gap-2 cursor-pointer p-2 bg-neutral-950 rounded border border-neutral-800 hover:border-blue-500 transition-colors">
                <input type="checkbox" v-model="config.showWIP" class="accent-blue-500"> Work In Progress
              </label>
              <label class="flex items-center gap-2 cursor-pointer p-2 bg-neutral-950 rounded border border-neutral-800 hover:border-blue-500 transition-colors">
              ...
              <label class="flex items-center gap-2 cursor-pointer p-2 bg-neutral-950 rounded border border-neutral-800 hover:border-blue-500 transition-colors">
                <input type="checkbox" v-model="config.showRepoSize" class="accent-blue-500"> Repository Size
              </label>
              <label class="flex items-center gap-2 cursor-pointer p-2 bg-neutral-950 rounded border border-neutral-800 hover:border-blue-500 transition-colors">
                <input type="checkbox" v-model="config.showLastCommit" class="accent-blue-500"> Last Commit
              </label>
              ...
              <label class="flex items-center gap-2 cursor-pointer p-2 bg-blue-900/20 rounded border border-blue-800 hover:border-blue-500 transition-colors md:col-span-2">
                <input type="checkbox" v-model="config.showQRCode" class="accent-blue-500"> 
                <span class="font-bold text-blue-400">Embed QR Code (Links to Repo)</span>
              </label>
            </div>
          </div>
      ...
      <div class="lg:col-span-7 flex flex-col gap-6">
        ...
        <div class="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 flex-1 flex flex-col items-center justify-center relative shadow-xl overflow-hidden custom-scrollbar">
          ...
          <div class="absolute top-6 left-6 right-6 flex justify-between items-center z-20">
            ...
            <button v-if="selectedRepo" @click="downloadImage" :disabled="downloading" class="flex items-center gap-2 bg-green-600 hover:bg-green-500 text-white text-xs font-bold px-4 py-2 rounded-lg transition-all shadow-lg active:scale-95 disabled:opacity-50">
          ...
          <div v-if="selectedRepo" class="mt-14 w-full overflow-x-auto custom-scrollbar flex items-start justify-center p-4">
            ...
            <div ref="cardRef" 
                 ...
                 class="rounded-2xl shadow-2xl text-center relative z-10 border border-black/5 flex-shrink-0 transition-all duration-75 flex flex-col">
              
              <div :style="{ backgroundColor: config.imageBg }" 
                   class="p-0 rounded-xl mb-5 border border-black/5 flex items-center justify-center overflow-hidden h-48 relative">
                <img :src="finalImageUrl" 
                     class="transition-all duration-75 rounded-xl" 
                     :style="{ 
                       width: `${imgScale * 100}%`, 
                       height: `${imgScale * 100}%`,
                       maxWidth: 'none',
          ...
          <div v-else class="text-neutral-600 italic mt-10">
        ...
        <div v-if="selectedRepo" class="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 relative">
          <div class="flex justify-between items-center mb-3">
            <h2 class="text-sm font-bold text-neutral-500 uppercase tracking-widest">README Embed Code</h2>
            <button @click="copy" class="text-xs font-bold text-blue-500 bg-blue-500/10 px-3 py-1 rounded hover:bg-blue-500/20">
              {{ copied ? 'COPIED!' : 'COPY' }}
            </button>
          </div>
